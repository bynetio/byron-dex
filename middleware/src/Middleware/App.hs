...
import           Colog.Polysemy.Formatting      (WithLog, addThreadAndTimeToLog, cmap, logInfo, logTextStderr,
                                                 newLogEnv, renderThreadTimeMessage)
...
import           Servant.Client.Streaming       (ClientError (FailureResponse))
...
createApp = do
  ...
  where
    ...
    handleErrors (Left (HttpError err)) = Left $ badRequest err
    ...
    badRequest :: ClientError -> ServerError
    badRequest (FailureResponse info resp) =
       err400
        { errBody = encode . DexTypes.Error . toStrict . decodeUtf8 . responseBody $ resp
        , errHeaders = contentTypeJson
        }
    badRequest _ =
       err400
        { errBody = encode $ DexTypes.Error "Bad Request"
        , errHeaders = contentTypeJson
        }
