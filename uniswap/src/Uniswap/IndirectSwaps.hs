...
indirectSwapPools :: forall s. Eff '[IndirectSwapAction s] [LiquidityPoolWithCoins]
indirectSwapPools = send $ IndirectSwapPools @s

indirectSwapStep :: forall s. s -> (LiquidityPoolWithCoins, (Amount A, Amount B)) -> Eff '[IndirectSwapAction s] s
...
iswap IndirectSwapParams {..} = do
  ...
  transaction <-
    foldM (indirectSwapStep @s) initState $
